...
def myPutText(src, text, pos, font_size, font_color):
    ...
    # 윈도우 에서는 'fonts/gulim.ttc', 애플 "AppleGothic.ttf"
...
def detect_ML(input_array):
    ...
    pred = np.argmax(pred)
    result = labels["val"][pred]
    
    # Check if the result is a single consonant
    if result in double_const.keys():
        # Use the previous result and current result to determine the double consonant
        if prev_res_final == double_const[result]:
            return result + prev_res_final
...
while True:
    ...
    with mp_hands.Hands(
        ...
        if cur_res_final and cur_res_final != prev_res_final:
            ...
            chars = list(set(final_type_store))
            char_to_ix = { ch:i for i,ch in enumerate(chars) }
            ix_to_char = { i:ch for i,ch in enumerate(chars) }
            jamo_numbers = [char_to_ix[x] for x in final_type_store]
